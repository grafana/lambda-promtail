...
func parseCognitoRecord(record Record) (logproto.Entry, error) {
	...
	if err != nil {
		return logproto.Entry{}, fmt.Errorf("failed to marshal cognito record content: %w", err)
	}
	if val, ok := record.Content["eventTimestamp"]; ok {
		sec, nsec, err := getUnixSecNsec(val.(string))
		if err != nil {
			return logproto.Entry{}, fmt.Errorf("failed to parse cognito eventTimestamp %q: %w", val, err)
		}
